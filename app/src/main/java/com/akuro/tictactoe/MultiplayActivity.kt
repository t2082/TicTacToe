...
class MultiplayActivity : AppCompatActivity() {
    ...
    private fun onCellClicked(cell: TextView, row: Int, col: Int) {
        ...
        if (isGameOver(getBoardState()) == 1) {
            ...
            showOverDialog(2)
        }else if (isGameOver(getBoardState()) == 3){
            showOverDialog(3)
        }
    }
    ...
    private fun showOverDialog(status: Int){
        ...
        try {
            ...
            if(status == 1){
                builder.setMessage("Player 2 thắng !")
            }
            else if(status == 2){
                builder.setMessage("Player 1 thắng !")
            }
            else{
    ...
    private fun isGameOver(board: Array<Array<Char>>): Int {
        if(evaluate(board) == +10) return 1
        else if(evaluate(board) == -10) return 2
        else if(board.all { row -> row.all { cell -> cell != ' ' } }) return 3
        else return 0
    }
